...
impl<'a> Scanner<'a> {
  ...
  pub(super) fn number(&mut self) -> Token<'a> {
    ...
    let dot_index = match self.rest.char_indices().find(|(_, c)| !c.is_ascii_digit()) {
      None => {
        let token = Token::new(TokenKind::Number, Some(self.rest), self.line);
        self.rest = "";
        return token;
      }
      Some((index, c)) => {
        if c != '.' || self.rest[index + 1..].is_empty() {
          ...
          return Token::new(TokenKind::Number, Some(lexeme), self.line);
        }
        index
      }
    };
    ...
    Token::new(TokenKind::Number, Some(lexeme), self.line)
  }
}
...
mod tests {
  ...
  fn test_number_parsing() {
    ...
    let expected = [
      (TokenKind::Number, Some("123"), 2),
      (TokenKind::Number, Some("45.67"), 3),
      (TokenKind::Dot, None, 4),
      (TokenKind::Number, Some("89"), 4),
      (TokenKind::Number, Some("12"), 5),
      (TokenKind::Dot, None, 5),
      (TokenKind::Number, Some("34.56"), 6),
      (TokenKind::Identifier, Some("x"), 6),
      (TokenKind::Number, Some("78.9"), 7),
      (TokenKind::Dot, None, 7),
      (TokenKind::Number, Some("0"), 7),
      (TokenKind::Number, Some("0.0"), 8),
      (TokenKind::Number, Some("123"), 9),
      (TokenKind::Identifier, Some("e45"), 9),
      (TokenKind::Number, Some("12"), 10),
      (TokenKind::Identifier, Some("_34"), 10),
      (TokenKind::Number, Some("12.34"), 11),
      (TokenKind::Dot, None, 11),
      (TokenKind::Number, Some("56.78"), 11),
      (TokenKind::Number, Some("56"), 12),
      (TokenKind::Dot, None, 12),
      (TokenKind::Dot, None, 13),
      (TokenKind::Number, Some("999999.999999"), 14),
      (TokenKind::Number, Some("12.3"), 15),
      (TokenKind::Dot, None, 15),
      (TokenKind::Number, Some("4"), 15),
    ];
    ...
    let edge_cases = [
      ...
      ("123", vec![(TokenKind::Number, Some("123"), 1)]),
      (
        "12.3.4",
        vec![
          (TokenKind::Number, Some("12.3"), 1),
          (TokenKind::Dot, None, 1),
          (TokenKind::Number, Some("4"), 1),
        ],
      ),
    ];
